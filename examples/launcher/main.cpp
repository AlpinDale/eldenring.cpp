...
const char* kLogPath = "erd_example_launcher.log";
...
bool get_registry_string(HKEY root, const char* subkey, const char* value, std::string& out) {
  char buf[4096] = {};
  DWORD type = 0;
...
void collect_library_paths(const std::string& vdf, std::vector<std::string>& out) {
  size_t pos = 0;
  while (true) {
...
bool find_elden_ring_via_steam(std::string& out_exe) {
  ...
  if (!get_registry_string(HKEY_CURRENT_USER, "Software\\Valve\\Steam", "SteamPath", steam_path)) {
    get_registry_string(HKEY_LOCAL_MACHINE, "SOFTWARE\\WOW6432Node\\Valve\\Steam", "InstallPath",
                        steam_path);
  }
  if (steam_path.empty()) {
    return false;
...
void print_last_error(const char* msg) {
  ...
  if (!detail.empty()) {
    std::fprintf(stderr, "%s (err=%lu: %s)\n", msg, static_cast<unsigned long>(err),
                 detail.c_str());
    log_line("%s (err=%lu: %s)", msg, static_cast<unsigned long>(err), detail.c_str());
  } else {
...
bool inject_dll(HANDLE process, const char* dll_path) {
  ...
  void* remote = VirtualAllocEx(process, nullptr, len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
  if (!remote) {
  ...
  SIZE_T written = 0;
  if (!WriteProcessMemory(process, remote, dll_path, len, &written) || written != len) {
  ...
  if (GetExitCodeThread(thread, &exit_code)) {
    log_line("LoadLibraryA returned 0x%lx", static_cast<unsigned long>(exit_code));
    if (exit_code == 0) {
...
int main(int argc, char** argv) {
  log_line("=== Example launcher start ===");
  ...
  if (dll_path.empty()) {
    std::fprintf(stderr, "Missing --dll\n");
    print_usage();
    return 1;
  }
  ...
  if (exe_path.empty()) {
    if (!find_elden_ring_via_steam(exe_path)) {
      std::fprintf(stderr, "Failed to find \"eldenring.exe\" (use --exe)\n");
...

  {
    ...
    if (GetModuleFileNameA(nullptr, self_path, static_cast<DWORD>(sizeof(self_path))) != 0) {
  ...
  if (!CreateProcessA(exe_path.c_str(), nullptr, nullptr, nullptr, FALSE, CREATE_SUSPENDED, nullptr,
